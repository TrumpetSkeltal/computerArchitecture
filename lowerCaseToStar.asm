...
main:
	...
	li $t0, 'a' # we hold these characters to tell whether the current char that we iterated to is upper or lower case.
	li $t1, 'z'
...
loop_begin: 
	...
	blt $t4, $t0, increment_ptr # if true, the current char is not a lowercase letter, we move onto increment_ptr
	bgt $t4, $t1, increment_ptr# if true, the current char is not a lowercase letter, we move onto increment_
	
	li $t4, '*'  # otherwise the char is a lowercase letter - we change it to '*'
